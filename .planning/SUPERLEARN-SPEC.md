...
- **Deliberate deviation from the literal spreadsheet formula:** the sheet's priority formula (`Importance/Challenge × (1+%Mastered)`) technically *rewards* higher mastery, which inverts what you actually want. The app's existing weakest-first sort (`showProgress()`, `seenDoms.sort(...)`) already does the sensible thing — keep that, don't port the sheet's formula literally. (The rebuilt `learning-system.xlsx` Phase 1 tab uses the corrected direction: `Importance/Challenge × (1 + mastery gap)`.)
...
```
**Admission order for new cards:** don't use raw bank order — use the same priority signal Phase 1 already surfaces (highest domain weight × lowest current mastery first). This closes the loop between Phase 1 and Phase 5 for free, since that ranking already exists in `showProgress()`'s `byDom`/`accOf`. `admissionOrder()` should return `BANK.questions` filtered to never-started cards, sorted by that priority.
